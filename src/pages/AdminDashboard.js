...
function AdminDashboard({ admin, onLogout }) {
  ...
  const changePassword = async (e) => {
    ...
    try {
      const token = getToken();
      await axios.post(`${BACKEND_URL}/api/auth/admin/change-password`, { currentPassword, newPassword }, { headers: { Authorization: `Bearer ${token}` } });
      setCurrentPassword(''); setNewPassword(''); setConfirmPassword('');
      setSuccess('Senha alterada com sucesso. Use a nova senha no próximo login.');
    } catch (err) { setError(err.response?.data?.error || 'Não foi possível alterar a senha.'); }
