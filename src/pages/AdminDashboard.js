...
function AdminDashboard({ admin, onLogout }) {
  ...
  const [approvedDriversOnlineBase, setApprovedDriversOnlineBase] = useState(0);
  ...
  const loadOnlineDrivers = async () => {
    ...
    if (!token) return;
    try {
      const response = await axios.get(`${BACKEND_URL}/api/drivers/online-count`, {
        headers: { Authorization: `Bearer ${token}` },
        timeout: 10000,
      });
      setOnlineDrivers(Number(response.data?.online) || 0);
      setApprovedDriversOnlineBase(Number(response.data?.approved) || 0);
    } catch (err) {
      // A contagem é atualizada silenciosamente para não interromper o painel caso o backend esteja indisponível momentaneamente.
    }
  };
  ...
  useEffect(() => {
    loadApplications();
    loadOnlineDrivers();
    const interval = setInterval(loadOnlineDrivers, 5000);
