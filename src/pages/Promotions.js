...
const DEFAULT_PROMOS = [
  { id: 1, code: 'VERÃO20', description: 'Ganhe 20% de desconto em suas próximas 5 corridas', discount: 20, type: 'percentage', expiresIn: '7 dias', icon: '🌞', used: false },
  { id: 2, code: 'NOVO50', description: 'R$ 50 de desconto em sua primeira corrida', discount: 50, type: 'fixed', expiresIn: '30 dias', icon: '🎁', used: false },
  { id: 3, code: 'AMIGOS10', description: 'Indique amigos e ganhe R$ 10 de crédito', discount: 10, type: 'referral', expiresIn: 'Ilimitado', icon: '👥', used: false }
];

function Promotions({ userId, onApplyCode }) {
  const storageKey = `pf17_promo_${userId || 'guest'}`;
  const [promos] = useState(DEFAULT_PROMOS);
  ...
  useEffect(() => {
    try {
      ...
      if (saved?.code) setAppliedCode(saved);
    } catch (_) {}
  }, [storageKey]);
  ...
  return (
    <div className="promotions-container">
      ...
      <div className="promos-list">{promos.map((promo) => <div key={promo.id} className={`promo-card ${promo.used ? 'used' : ''} ${appliedCode?.id === promo.id ? 'applied' : ''}`}><div className="promo-icon">{promo.icon}</div><div className="promo-content"><div className="promo-header"><h4>{promo.code}</h4><span className="discount-badge">{promo.type === 'percentage' && `${promo.discount}%`}{promo.type === 'fixed' && `R$ ${promo.discount}`}{promo.type === 'referral' && '🔗'}</span></div><p className="promo-description">{promo.description}</p><span className="expires">⏰ Expira em: {promo.expiresIn}</span></div><button type="button" className={`btn-use ${appliedCode?.id === promo.id ? 'applied' : ''}`} onClick={() => handleApplyPromo(promo)} disabled={promo.used}>{appliedCode?.id === promo.id ? '✓ Aplicado' : 'Usar'}</button></div>)}</div>
    </div>
  );
}
