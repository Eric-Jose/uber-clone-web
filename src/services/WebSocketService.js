...
class WebSocketService {
  ...
  connect() {
    ...
    this.authToken = token;
    this.socket = io(BACKEND_URL, { auth: { token }, transports: ['websocket', 'polling'], timeout: 10000, reconnection: true, reconnectionDelay: 1000, reconnectionDelayMax: 5000, reconnectionAttempts: 15, closeOnBeforeunload: true });
  ...
  ensurePassengerLocationWatch(rideId) {
    ...
    this.passengerLastLocation = null;
    this.passengerWatchId = navigator.geolocation.watchPosition((position) => {
      ...
      this.passengerLastLocation = { lat: latitude, lng: longitude };
  ...
  syncDriverLocationHttp(driverId, latitude, longitude) {
    ...
    void fetch(`${BACKEND_URL}/api/drivers/${encodeURIComponent(driverId)}/status`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ isOnline: true, currentLocation: { lat: Number(latitude), lng: Number(longitude) } }), cache: 'no-store' }).catch(() => {});
  }
  ...
  syncPassengerLocationHttp(rideId, latitude, longitude) {
    ...
    void fetch(`${BACKEND_URL}/api/rides/${encodeURIComponent(rideId)}/passenger-location`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ location: { lat: Number(latitude), lng: Number(longitude) } }), cache: 'no-store' }).catch(() => {});
  }
  ...
  async startRide(rideId) {
    return this.updateRideStatus(rideId, 'IN_PROGRESS', 'Não foi possível iniciar a corrida.');
  }

  async endRide(rideId) {
    return this.updateRideStatus(rideId, 'COMPLETED', 'Não foi possível finalizar a corrida.');
  }

  async cancelRide(rideId, reason = 'Cancelada pelo usuário') {
    return this.updateRideStatus(rideId, 'CANCELLED', 'Não foi possível cancelar a corrida.', reason);
  }

  async updateRideStatus(rideId, status, fallback, cancellationReason) {
    const token = localStorage.getItem('token');
    if (!token || !rideId) return null;
    const response = await fetch(`${BACKEND_URL}/api/rides/${encodeURIComponent(rideId)}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ status, ...(status === 'CANCELLED' ? { cancellationReason } : {}) }), cache: 'no-store' });
    const data = await response.json().catch(() => ({}));
  ...
  onRideStarted(callback) { const socket = this.ensureSocket(); if (!socket) return; socket.on('ride-started', callback); socket.on('ride_in_progress', callback); return socket; }
  ...
  off(event, callback) { if (!this.socket) return; this.socket.off(event, callback); if (event === 'ride-started') this.socket.off('ride_in_progress', callback); if (event === 'ride-ended') this.socket.off('ride-completed', callback); }
}
