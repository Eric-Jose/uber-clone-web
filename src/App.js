...
function App() {
  ...
  switch (currentPage) {
    ...
    default: return <div className="home-page"><div className="home-container"><div className="home-badge">🚗 PreçoFixo17</div><h1><span>Preço</span><strong>Fixo17</strong></h1><p>Corridas particulares com preço justo, segurança e conforto para você chegar ao seu destino.</p><div className="home-buttons"><button type="button" onClick={() => setCurrentPage('login')} className="btn-home">👤 Entrar como usuário</button><button type="button" onClick={() => setCurrentPage('register')} className="btn-home secondary">✨ Criar conta</button><button type="button" onClick={() => setCurrentPage('admin-login')} className="btn-home admin">🔐 Administrador</button></div></div></div>;
  }
}
export default App;
