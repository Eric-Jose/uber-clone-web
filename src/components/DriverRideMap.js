...
export default function DriverRideMap({ driverLocation, passengerLocation, destinationLocation, status }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      driverMarkerRef.current = null; targetMarkerRef.current = null; routeLayerRef.current = null;
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (!map) return;
    map.invalidateSize();
    const markerIcon = (className, label) => L.divIcon({ className: 'driver-map-pin-wrapper', html: `<div class="driver-map-pin ${className}"><span>${label}</span></div>`, iconSize: [48, 48], iconAnchor: [24, 24] });
    if (driver) {
      if (!driverMarkerRef.current) driverMarkerRef.current = L.marker([driver.lat, driver.lng], { icon: markerIcon('driver-pin', '🚙'), zIndexOffset: 1000 }).addTo(map);
      else driverMarkerRef.current.setLatLng([driver.lat, driver.lng]);
      driverMarkerRef.current.bindTooltip('Você', { direction: 'top', offset: [0, -22], opacity: 0.96, className: 'driver-map-tooltip' });
    }
    if (target) {
      ...
      if (!targetMarkerRef.current) targetMarkerRef.current = L.marker([target.lat, target.lng], { icon: markerIcon(pinClass, label), zIndexOffset: 900 }).addTo(map);
      else { targetMarkerRef.current.setLatLng([target.lat, target.lng]); targetMarkerRef.current.setIcon(markerIcon(pinClass, label)); }
      targetMarkerRef.current.bindTooltip(title, { direction: 'top', offset: [0, -22], opacity: 0.96, className: 'driver-map-tooltip' });
  ...
  useEffect(() => {
    ...
    const drawRoute = (latLngs, fallback = false) => {
      ...
      group.addLayer(L.polyline(latLngs, { color: '#090c0f', weight: 11, opacity: 0.72, lineCap: 'round', lineJoin: 'round' }));
      group.addLayer(L.polyline(latLngs, { color: '#ff6b00', weight: fallback ? 5 : 6, opacity: 0.98, dashArray: fallback ? '10 8' : undefined, lineCap: 'round', lineJoin: 'round' }));
    ...
    fetch(url, { signal: controller.signal })
      ...
      .then((data) => {
        ...
        const latLngs = coordinates.map(([lng, lat]) => [Number(lat), Number(lng)]).filter(([lat, lng]) => Number.isFinite(lat) && Number.isFinite(lng));
        if (!latLngs.length) fallback(); else drawRoute(latLngs, false);
      })
  ...
  return (
    <div className="driver-map-shell">
      ...
      <style>{`
        ...
        .driver-map-topbar span:last-child{margin-left:auto;color:#aab4ba;font-weight:700;letter-spacing:0;text-transform:none}.driver-map-live-dot{width:8px;height:8px;border-radius:50%;background:#27c96f;box-shadow:0 0 0 5px rgba(39,201,111,.12),0 0 14px rgba(39,201,111,.42)}
        .driver-map-caption{position:absolute;left:12px;right:12px;bottom:12px;z-index:500;display:flex;align-items:center;gap:8px;background:rgba(3,6,8,.91);color:#fff;border:1px solid rgba(255,107,0,.74);border-radius:15px;padding:10px 12px;box-shadow:0 8px 25px rgba(0,0,0,.42);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);font:800 12px/1.15 Arial,sans-serif}.driver-map-caption b{font-size:12px}.driver-map-caption span{margin-left:auto;color:#c0c8cd;font-size:10px;font-weight:700}
        ...
        .driver-map-tooltip{background:#05080a!important;color:#fff!important;border:1px solid #303a41!important;border-radius:9px!important;box-shadow:0 8px 20px rgba(0,0,0,.45)!important;font:800 11px/1 Arial,sans-serif!important;padding:6px 8px!important}.driver-map-tooltip:before{border-top-color:#05080a!important}.driver-map-shell .leaflet-control-zoom{border:1px solid #303a41!important;border-radius:12px!important;overflow:hidden;box-shadow:0 10px 25px rgba(0,0,0,.46)!important;margin-top:74px!important;margin-right:12px!important}.driver-map-shell .leaflet-control-zoom a{width:40px!important;height:40px!important;line-height:38px!important;background:rgba(5,8,10,.92)!important;color:#fff!important;border:0!important;border-bottom:1px solid #303a41!important;font-weight:900!important}.driver-map-shell .leaflet-control-zoom a:last-child{border-bottom:0!important}.driver-map-shell .leaflet-control-zoom a:hover{background:#ff6b00!important;color:#fff!important}.driver-map-shell .leaflet-control-attribution{background:rgba(3,6,8,.70)!important;color:#7f8b93!important;font-size:9px!important}.driver-map-shell .leaflet-control-attribution a{color:#ff8b1f!important}.driver-map-shell .leaflet-tile-pane{filter:none!important}
...
// Mapa permanente do motorista: aparece assim que o painel do motorista entra no DOM.
// Quando existe uma corrida, o mapa completo acima assume a navegação e este é ocultado.
...
function startIdleDriverMap() {
  ...
  const getUser = () => { try { return JSON.parse(localStorage.getItem('user') || 'null'); } catch (_) { return null; } };
  ...
  const ensureHost = () => {
    ...
    if (!host) {
      ...
      if (firstBox && firstBox.parentNode) firstBox.parentNode.insertBefore(host, firstBox.nextSibling); else panel.appendChild(host);
    }
    return host;
  };

  const clearRoute = () => { if (routeLayer && map) map.removeLayer(routeLayer); routeLayer = null; };

  const drawRoute = async (driver, target) => {
    if (!map || !driver || !target) return;
    clearRoute();
    try {
      const response = await fetch(`${ROUTE_URL}${driver.lng},${driver.lat};${target.lng},${target.lat}?overview=full&geometries=geojson&steps=false`);
  ...
  const updateTarget = (ride) => {
    ...
    if (!targetMarker) targetMarker = L.circleMarker([target.lat,target.lng], { radius:10, color:'#fff', weight:3, fillColor:status === 'IN_PROGRESS' ? '#27c96f' : '#ff6b00', fillOpacity:1 }).addTo(map);
    else { targetMarker.setLatLng([target.lat,target.lng]); targetMarker.setStyle({ fillColor:status === 'IN_PROGRESS' ? '#27c96f' : '#ff6b00' }); }
  ...
  const initMap = () => {
    ...
    if (!canvas) return;
    map = L.map(canvas, { zoomControl:false, attributionControl:true, preferCanvas:true }).setView([-14.235,-51.925],5);
  ...
  const pollRide = async () => {
    ...
    if (!token) return;
    try {
      const response = await axios.get(`${BACKEND_URL}/api/rides/active`, { headers:{ Authorization:`Bearer ${token}` }, timeout:8000 });
      const ride = response.data && response.data.ride;
      if (!ride) {
        activeRideId = null;
  ...
  const boot = () => {
    ...
    refresh();
    if (!ridePollTimer) ridePollTimer = window.setInterval(pollRide, 2500);
    if (!observer) {
