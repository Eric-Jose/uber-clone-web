...
export default function DriverRideMap({ driverLocation, passengerLocation, destinationLocation, status }) {
  ...
  useEffect(() => {
    ...
    const markerIcon = (className, label) => L.divIcon({
      ...
      html: `<div class="driver-map-pin ${className}">${label}</div>`,
      iconSize: [40, 40],
      iconAnchor: [20, 20]
    });
    ...
    if (driver) {
      if (!driverMarkerRef.current) {
        driverMarkerRef.current = L.marker([driver.lat, driver.lng], { icon: markerIcon('driver-pin', '🚙') }).addTo(map);
      } else {
    ...
    if (target) {
      ...
      if (!targetMarkerRef.current) {
        targetMarkerRef.current = L.marker([target.lat, target.lng], { icon: markerIcon(status === 'IN_PROGRESS' ? 'destination-pin' : 'passenger-pin', label) }).addTo(map);
      } else {
        targetMarkerRef.current.setLatLng([target.lat, target.lng]);
        targetMarkerRef.current.setIcon(markerIcon(status === 'IN_PROGRESS' ? 'destination-pin' : 'passenger-pin', label));
      }
  ...
  useEffect(() => {
    ...
    fetch(url, { signal: controller.signal })
      ...
      .then((data) => {
        if (requestId !== routeRequestRef.current || !data.routes || !data.routes[0]) return;
  ...
  return (
    <div className="driver-map-shell">
      ...
      <div className="driver-map-caption">
        {status === 'IN_PROGRESS' ? 'Rota até o destino' : 'Rota até o passageiro'}
      </div>
      <style>{`
        .driver-map-shell{position:relative;margin-top:14px;border-radius:18px;overflow:hidden;border:1px solid #e5e7eb;background:#eef2f7;box-shadow:0 8px 24px rgba(0,0,0,.08)}
        .driver-ride-map{height:390px;width:100%;z-index:1}
        .driver-map-caption{position:absolute;left:12px;bottom:12px;z-index:500;background:#111827;color:#fff;border-radius:999px;padding:9px 13px;font:800 12px/1 Arial,sans-serif;box-shadow:0 4px 16px rgba(0,0,0,.2)}
        .driver-map-pin-wrapper{background:transparent;border:0}
        .driver-map-pin{width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:3px solid #fff;box-shadow:0 3px 12px rgba(0,0,0,.3);font-size:19px}
        .driver-pin{background:#111827}.passenger-pin{background:#ff6a00}.destination-pin{background:#dc2626}
        @media(max-width:640px){.driver-ride-map{height:320px}.driver-map-caption{font-size:11px}}
      `}</style>
    </div>
  );
}
