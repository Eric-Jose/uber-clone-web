...
// SEGURANÇA: nenhuma credencial do Firebase Admin (chave privada, e-mail da
// service account) deve ficar hardcoded no código. Todos os segredos vêm
// exclusivamente das variáveis de ambiente. Se não estiverem configuradas, o
// backend cai automaticamente no modo mock (in-memory) usado em preview/dev.
...
const hasFirebaseCredentials = Boolean(rawPrivateKey && rawClientEmail && projectId);
...
if (!admin.apps.length) {
  ...
  if (hasFirebaseCredentials) {
    ...
    console.warn('⚠️ PreçoFixo17: credenciais do Firebase Admin ausentes. Configure FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL e FIREBASE_PRIVATE_KEY.');
  }
...
io.use((socket, next) => {
  try {
    ...
    socket.user = jwt.verify(token, require('./config/secrets').JWT_SECRET);
    next();
