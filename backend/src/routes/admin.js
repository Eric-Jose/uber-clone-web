...
router.get('/drivers/applications', async (_req, res) => {
  try {
    ...
    const applications = drivers.map(driver => {
      ...
      return {
        uid: String(driver.userId),
        fullName: driver.fullName || user?.name || 'Sem nome',
        email: user?.email || '',
        phone: user?.phone || '',
...
router.patch('/drivers/:uid/approval', async (req, res) => {
  try {
    ...
    if (status === 'approved') {
      ...
      driver.rejectionReason = String(req.body?.reason || 'Cadastro rejeitado pelo administrador.');
      driver.isOnline = false;
    } else {
      driver.approvedAt = null;
      driver.rejectedAt = null;
...
router.get('/admin-stats/overview', async (_req, res) => {
  try {
    ...
    const [passengers, drivers, onlineDrivers, ridesToday, activeRides, completedToday, cancelledToday, revenueAgg] = await Promise.all([
      User.countDocuments({ userType: 'passenger', isActive: true }),
      Driver.countDocuments({}),
      Driver.countDocuments({ status: 'approved', isOnline: true }),
      Ride.countDocuments({ createdAt: { $gte: start, $lt: end } }),
      Ride.countDocuments({ status: { $in: ['SEARCHING', 'ACCEPTED', 'IN_PROGRESS'] } }),
      Ride.countDocuments({ status: 'COMPLETED', completedAt: { $gte: start, $lt: end } }),
      Ride.countDocuments({ status: 'CANCELLED', updatedAt: { $gte: start, $lt: end } }),
      Ride.aggregate([
    ...
    const dailyAgg = await Ride.aggregate([
      { $match: { createdAt: { $gte: since, $lt: end } } },
      { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$createdAt' } }, rides: { $sum: 1 } } },
      { $sort: { _id: 1 } }
    ]);
    ...
    for (let i = 0; i < 7; i++) {
      ...
      daily.push({ date: key, rides: dailyMap.get(key) || 0 });
    }
