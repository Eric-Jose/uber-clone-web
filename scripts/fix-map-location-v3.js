...
const newSearch = [
  ...
  '    // Every new text starts a fresh search. The previous destination is',
  '    // invalidated immediately so the passenger can search again any time.',
  '    setDestinationCoords(null);',
  '    setSearching(true);',
  '    const term = value.trim();',
  '    const query = encodeURIComponent(term);',
  '    const searchUrl = `${B}/api/location/search?q=${query}`;',
  '    fetch(searchUrl, { headers: { Accept: \'application/json\' } })',
  '      .then(async (response) => {',
  '        if (!response.ok) throw new Error(`Busca de endereço: ${response.status}`);',
  ...
  '        const seen = new Set();',
  '        const ordered = results.filter((item) => {',
  ...
  '        if (!ordered.length) setError(\'Nenhum local encontrado. Tente escrever o endereço completo.\');',
  '      })',
  '      .catch(() => {',
  '        if (requestId === searchRequestRef.current) {',
  '          setSuggestions([]);',
  '          setError(\'Não foi possível pesquisar este local agora. Tente novamente.\');',
  '        }',
  '      })',
...
console.log('[fix-map-location-v3] GPS automático, OpenStreetMap sem API key e busca mundial de destinos via proxy backend, com pesquisa repetível e qualquer local retornado pelo geocodificador.');
